# Mission Control — THE COMPLETE FEATURE LIST (for rebuilding from scratch)

> Compiled 2026-09-02 from every record in this repo: `TASKS.md` (all 10,409 lines), `ROADMAP.md`,
> `CLAUDE.md`, `WIRING.md`, `docs/planning/MASTER-MEMORY-PLAN.md` + the Slow Plan pages P0–P7,
> `docs/dev-room/*`, `docs/*.md`, the audits, and the actual code of the web app, the PC bridge,
> Home Base and the Dev Room. Nothing was summarized away on purpose: if a feature exists anywhere
> in the plan or the code, it is on this list.
>
> **How to read it.** Every line carries a status mark so a rebuild knows what was real and what
> was only planned:
> ✅ built · 🟢 confirmed working live by Issac · 🔄 half-built / partly real · ⬜ planned, not built ·
> ⏭ parked · ⏸ built but waiting on Issac's PC / eyes to prove.
> Status is honest to the 2026-09-02 professional audit (421 findings). Where the audit found a
...
1. **OpenClaw** — the open-source personal AI assistant (https://openclaw.ai) that runs on the
   owner's Windows PC. It is the brain-runner. Not in this repo. GPT-5.5 via the ChatGPT plan
   (no API key), Telegram bot, gateway on `ws://127.0.0.1:18789`.
...
- Plain, natural language everywhere in the product. Say "OpenClaw", never "engine".
...
└── Aaron — the CEO AGENT: runs PERSONAL behind a hard wall, sees everything
    ...
    └── Maria — Office Manager / Chief of Staff: runs the whole business floor.
        ...
        ├── Max — Maintenance lead (room "Maintenance"): keeps OpenClaw and the pipes healthy
...
- **Dexter** 🟢 — Development room lead. A Claude Code session on the PC (NOT an OpenClaw agent), talked
...
1. **Vault** — keys/logins stored once ON THE PC with OpenClaw; agents request a capability, never
...
### 2.9 Every room = its own OpenClaw agent ✅ built / ⏸ flip never run
Rooms get their own agent (folder + settings, `agent:room-<slug>:main`); workers are light sessions
(`…:w-<name>`); Maria = `agent:main:main` with `dmScope=main` so Telegram/voice/website are one thread;
police `agent:main:mc-police`. Today the office still runs on ONE agent with per-room session keys.
...
The UI = buttons over OpenClaw's OWN setup. Three doors: native OpenClaw connectors → MCP adapter
...
- **Top bar**: mobile menu · real time-of-day greeting with the owner's name + today's date (Hebrew
  locale when set) or the screen's title/subtitle · OpenClaw status pill (live / off, from the PC
...
- **OpenClaw status card** on top (see 3.24).
...
- **Recurring jobs** panel from OpenClaw's `cron list` (count / "Can't see them right now" when the PC
...
- Card kinds: room work "For your OK" (a task's report) · outgoing **drafts** (real to / subject /
  ...
  asks ("🔧 Maria asks: restart OpenClaw — reason") · Dexter's office-wide rule proposals · code
...
- **The store** ("the store of everything"): search (clear, summary) · tabs All / Services / API / MCP /
  ...
  services row · live OpenClaw tiles born from the PC's own skills catalog with truth pills (Connected
...
- **Spend over time**: 7-day bars (tooltip = witnessed tokens + labelled $ estimate; "no spend
  ...
  yet). **Live from OpenClaw** card (real total tokens + per-agent token rows from the fresh snapshot).
...
Sources column (All documents + each source; first REAL source = "This PC · OpenClaw workspace",
...
Real folder tree of the PC's OpenClaw workspace (read-only walker: depth 4, 400 entries, skips
...
Council debate scene · wire the office to live OpenClaw state · office-only cost meter · quick-capture
...
### 5.1 Her permanent personality (`persona.mjs`, loaded into OpenClaw) ✅
...
A plain Node program (no dependencies) that lives on the Windows PC next to OpenClaw. Home Base runs
...
### 6.1 Loops ✅
- **Tick every 20 s** (overlap-guarded, the fix for the "30 node.exe probe storm"): read OpenClaw
  (`sessions --json`, `agents list`, `health` with 3 retries, `cron list --json`) → build the
  **snapshot** → push it to the cloud row `openclaw_live/snapshot` (HMAC-signed when an office lock
...
`set_secret` (only when no lock exists) · `rooms_migrate` (one agent per room: back up openclaw.json,
...
authorize / verifyauth / authhelp / verify through OpenClaw's own Google skill) · channel setups and
...
`browser_setup` (OpenClaw's isolated managed browser; AliExpress) · `helper_setup` (the recipe lane:
...
### 6.4 Session keys ✅
Maria `agent:main:main` (dmScope=main); rooms `agent:main:mc-room-<slug>` today, `agent:room-<slug>:main`
after the flip; workers `…-w-<worker>`; police `agent:main:mc-police`. Brain pickers become real flags.
...
- **Files reporter**: bounded read-only walk of the OpenClaw workspace (400 entries, depth 4; md /
...
### 6.6 OpenClaw facts the rebuild relies on
OpenClaw v2026.6.10 pinned (PC runs 6.11), native Windows, Node 24; CLI used: `status` · `sessions
--json` · `agents list --json` · `health` · `agent --message --session-key --thinking --model` ·
`message send --channel telegram --target` · `gateway run / install / stop / restart` · `channels add /
login / status / capabilities / remove` · `mcp add / set / list / show / login / probe / configure /
reload` · `secrets configure / apply / audit / reload` · `skills list / inspect / install` · `plugins
install / enable / disable / inspect` · `approvals allowlist / get / set` · `exec-policy show / set /
preset` · `cron list --json` · `models list` · `sessions cleanup --fix-dm-scope` · `configure
--section` · `browser --browser-profile openclaw start / open / snapshot`. Decision: the engine STAYS
OpenClaw for the single-user build; a cloud Claude behind the same bridge/database seam is the Phase-4
option.
...
A quiet background helper so Issac never has to walk to the PC. Runs OpenClaw + the messenger as
...
### 7.1 Screens ✅
- **Home** ("Your office"): four lights (OpenClaw · Messenger · ChatGPT login · Memory; good / warn /
  bad / off; distinguishes "haven't heard yet" from "off"; 3-min freshness), whole-app glow, live
  **Start / Stop / Restart** buttons + 🩺 Health check (own window: openclaw doctor → All clear / A few
...
- **Set up my office** wizard (until complete): ① Get Node → ② install OpenClaw pinned
  `npm install -g openclaw@2026.6.10` → ③ sign in to ChatGPT (the engine's own login) → ④ start the
...
### 8.1 Locked shape ✅
- "The store of everything": tiles are born from OpenClaw's OWN skills catalog (a cloud row the PC
...
- D2 **AliExpress** (OpenClaw's isolated managed browser profile; human login; proof = account read) ✅
...
Ready services (grant-and-prove; OpenClaw skills that need nothing on the PC):
...
- **Dexter's tools**: send-only Telegram voice (`send`, `keyboard`, `status` pin-once-then-edit,
  ...
  (25 MB cap, ffmpeg conversion) using the OpenAI Audio key from OpenClaw's config.
...
- Wiring: REST (`/v3/documents` with container tags + metadata) + MCP registered into OpenClaw at
...
- **Live tables**: `mc_setup (owner_id, id, data jsonb, updated_at)` — the office record store, one
  row per store; `openclaw_live (owner_id, id, data, updated_at)` — the PC snapshot (`snapshot`);
...
- **Env vars**: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, `MC_SESSION_SECRET`,
  ...
  `OPENAI_API_KEY`, `TRIPO_API_KEY`, `SENTRY_DSN`; PC: `OPENCLAW_CMD`, `POLL_SECONDS`, `ENABLE_COMMANDS`,
...
· `mc.personality.v1` · `mc.engine.lastseen.v1` · `mc.openclaw.snapshot` · `mc.sm.saved` ·
...
`Home Base\office-state\*.json` (see 6.5) · `~/.openclaw/openclaw.json` (skills entries, MCP mounts,
dmScope, agents) · `~/.openclaw/agents/main/sessions/*.jsonl` (transcripts) · journals · Downloads
...
- **The progress page** (`/progress-html/`, a bright visual mirror of TASKS.md): "where we are" panel
  ...
  (memory brief, scenarios, OpenClaw-vs-Claude report, mockups).
...
per-room agents don't exist live · the office never reads OpenClaw. Standing lesson: a gap list is
...
Plain natural language always (rule #1, Issac is non-technical) · "OpenClaw", never "engine" ·
...
2026-06-15 first notes (OpenClaw v2026.6.6; "Octopus dashboard" idea; build from scratch chosen over
